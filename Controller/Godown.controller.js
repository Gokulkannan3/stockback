// Controller/Godown.controller.js
  ...
  try {
    ...
    for (let godown of godowns) {
      const stockResult = await pool.query(
        `SELECT id, product_type, productname, brand, current_cases, per_case, date_added, last_taken_date, taken_cases 
         FROM public.stock 
         WHERE godown_id = $1 
  ...
  try {
    ...
    if (existingStock.rows.length > 0) {
      ...
      const insertResult = await pool.query(
        'INSERT INTO public.stock (godown_id, product_type, productname, brand, current_cases, per_case) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id',
        [godown_id, product_type, productname, brand, casesAddedNum, per_case]
      );
...
exports.getStockByGodown = async (req, res) => {
  try {
    ...
    console.log('Stock data sent:', result.rows); // Debug log
...
exports.takeStockFromGodown = async (req, res) => {
  ...
  try {
    ...
    if (stockCheck.rows.length === 0) {
      console.log(`Stock entry not found for stock_id: ${stock_id}`);
...
exports.getStockHistory = async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT h.*, s.productname, s.brand, s.per_case * h.cases as per_case_total 
